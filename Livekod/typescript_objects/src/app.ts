...
// characters.map(character => console.log(character.occupation));
...
// Detta kallas object literal och för att använda det MÅSTE ni använda backticks ``
...
function compare(a : Character, b: Character) : number {
    const nameA = a.name;
    const nameB = b.name;
...
let sortedByName = characters.sort((a : Character, b : Character) => {
    if(a.name < b.name) {
        return 1;
